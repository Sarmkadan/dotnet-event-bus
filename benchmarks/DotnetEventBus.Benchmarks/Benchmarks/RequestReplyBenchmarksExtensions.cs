...
/// <summary>
/// Extension methods for <see cref="RequestReplyBenchmarks"/> providing additional benchmarking utilities
/// and convenience methods for common benchmarking scenarios.
/// </summary>
public static class RequestReplyBenchmarksExtensions
{
    /// <summary>
    /// Creates a new <see cref="RequestReplyBenchmarks"/> instance and runs a single benchmark iteration.
    ...
    /// <exception cref="ArgumentException"><paramref name="benchmarkName"/> is not a known benchmark.</exception>
    ...
    public static async Task RunSingleBenchmarkAsync(this RequestReplyBenchmarks benchmark, string benchmarkName)
    {
        ...
        _ = !string.IsNullOrWhiteSpace(benchmarkName) ? benchmarkName : throw new ArgumentException("Benchmark name cannot be null or whitespace.", nameof(benchmarkName));
    ...
    public static Dictionary<string, Task> RunAllBenchmarksSequentially(this RequestReplyBenchmarks benchmark)
    {
        ...
        var results = new Dictionary<string, Task>(StringComparer.Ordinal)
        {
            [nameof(RequestReplyBenchmarks.Request_Reply_Single)] = benchmark.Request_Reply_Single(),
            [nameof(RequestReplyBenchmarks.Request_Reply_10_Sequential)] = benchmark.Request_Reply_10_Sequential(),
            [nameof(RequestReplyBenchmarks.Request_Reply_10_Parallel)] = benchmark.Request_Reply_10_Parallel(),
            [nameof(RequestReplyBenchmarks.Request_Reply_With_Timeout)] = benchmark.Request_Reply_With_Timeout(),
            [nameof(RequestReplyBenchmarks.Request_Reply_Large_Payload)] = benchmark.Request_Reply_Large_Payload()
        };
    ...
    /// </summary>
    /// <param name="benchmark">The benchmarks instance.</param>
    /// <param name="batchSize">Number of requests to send sequentially. Must be positive.</param>
    /// <exception cref="ArgumentNullException"><paramref name="benchmark"/> is <see langword="null"/>.</exception>
    ...
    {
        _ = benchmark ?? throw new ArgumentNullException(nameof(benchmark));
        _ = batchSize > 0 ? batchSize : throw new ArgumentOutOfRangeException(nameof(batchSize), batchSize, "Batch size must be positive.");
    ...
    /// </summary>
    /// <param name="benchmark">The benchmarks instance.</param>
    /// <param name="concurrencyLevel">Number of concurrent requests to send. Must be positive.</param>
    /// <exception cref="ArgumentNullException"><paramref name="benchmark"/> is <see langword="null"/>.</exception>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="concurrencyLevel"/> is not positive.</exception>
    ...
    {
        _ = benchmark ?? throw new ArgumentNullException(nameof(benchmark));
        _ = concurrencyLevel > 0 ? concurrencyLevel : throw new ArgumentOutOfRangeException(nameof(concurrencyLevel), concurrencyLevel, "Concurrency level must be positive.");
    ...
    /// </summary>
    /// <param name="benchmark">The benchmarks instance.</param>
    /// <param name="testRequestId">Custom request ID to use for validation. Defaults to "validation-test".</param>
    /// <exception cref="ArgumentNullException"><paramref name="benchmark"/> is <see langword="null"/>.</exception>
    /// <exception cref="ArgumentException"><paramref name="testRequestId"/> is null or whitespace.</exception>
    /// <exception cref="InvalidOperationException">Validation checks fail (request ID mismatch, value processing error, or unexpected result).</exception>
